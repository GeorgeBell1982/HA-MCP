...
8. `yaml`: document-model parsing with Home Assistant custom-tag support and explicit non-resolution of `!secret`. Evaluate `yaml` (eemeli) during Phase 2 because it preserves document nodes/comments/order and supports custom tags; lock the decision with fixture evidence before structured editing.
...
| Git status/diff/commit                                         | Repository detection within mapped `/config`                   | Phase 2; no initialization or commit without explicit authorization.                                                                                                       |
