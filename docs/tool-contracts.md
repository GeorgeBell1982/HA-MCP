...
The requested system, entity, resource, config, error, Git, and proposal reads are read-only. List/search tools accept typed filters, a conservative limit, and opaque cursor. Phase 2 ha_read_config_file accepts only a strict repository-relative path and returns one bounded whole text file; ranges, caller byte limits, and follow-up references are not part of the frozen contract.
...
ha_propose_config_change accepts one bounded whole-document replacement for an existing YAML file, bound to an expected SHA-256 and idempotency UUID. It writes proposal metadata only, never live config. It returns the exact redacted diff, hashes, risk, validation plan, required approval, and reload/restart impact.
...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `ha_get_system_info`, `ha_list_entities`, `ha_get_entity_state`, `ha_search_entities`, API-feasible automation/script/helper/dashboard/scene/blueprint list/get operations, `ha_get_config_status`, `ha_get_recent_errors`                      | Documented REST/WebSocket or explicit API capability against HA OS/Core 2026.7.2 | Schemas freeze when shipped; unsupported sources return typed `capability_unavailable`. Storage-mode dashboard access must use a verified supported API, never `.storage` editing. |
| 2     | ha_list_config_files, ha_read_config_file, ha_search_config, ha_list_config_resources, ha_get_config_resource, ha_get_git_status, ha_get_git_diff, ha_list_proposals, ha_get_pending_diff, ha_propose_config_change, ha_discard_proposed_change | Confined /homeassistant, hardened Git, protected /data proposal store            | Frozen in phase2-contracts.md; unregistered until security gates pass. Dashboards stay on supported API or typed unavailable.                                                      |
