...
function operatorHtml() {
  return `<!doctype html><meta charset="utf-8"><title>HA Engineering MCP</title><style>body{font:16px system-ui;max-width:55rem;margin:2rem auto}button{margin:.25rem}pre{white-space:pre-wrap}</style><h1>Home Assistant Engineering MCP</h1><p id="status">Loading…</p><p><a href="certificate">Download public certificate</a></p><button data-action="pair">Pair new client</button><button data-action="rotate-certificate">Rotate certificate</button><h2>One-time credential</h2><pre id="secret">Not generated. It is never stored by this page.</pre><h2>Clients</h2><div id="clients"></div><script>const h={'x-ha-mcp-csrf':'1'};async function refresh(){const [health,fp,clients]=await Promise.all(['health','fingerprint','clients'].map(x=>fetch(x,{cache:'no-store'}).then(r=>r.json())));status.textContent='Healthy: '+health.ok+' — fingerprint: '+fp.fingerprint;window.clients.innerHTML=clients.clients.map(c=>c.clientId+' '+(c.revokedAt?'revoked':'active')+' <button data-action="rotate/'+c.clientId+'">rotate</button><button data-action="revoke/'+c.clientId+'">revoke</button>').join('<br>')}document.addEventListener('click',async e=>{const a=e.target.dataset?.action;if(!a)return;const r=await fetch(a,{method:'POST',headers:h,cache:'no-store'});if(r.headers.get('content-type')?.includes('json')){const v=await r.json();if(v.credential)secret.textContent=v.credential;if(v.fingerprint)alert('New fingerprint: '+v.fingerprint+'; restart the add-on and update bridge pins.')}await refresh()});refresh()</script>`;
}
